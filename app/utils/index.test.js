...
describe("getTotalDaysBetween1900AndYear(year)", () => {
  ...
  it("should return 365 for year 1901 (1900 is not leap year)", () => {
    // 1900 มี 365 วัน, 1901 มี 365 วัน
...
describe("isValidDate", () => {
  it("should return false for invalid month", () => {
    expect(isValidDate(2022, 0, 1)).toBe(false);
    expect(isValidDate(2022, 13, 1)).toBe(false);
  });

  it("should return false for invalid day", () => {
    expect(isValidDate(2022, 1, 0)).toBe(false);
    expect(isValidDate(2022, 1, 32)).toBe(false);
  });

  it("should return true for leap year February", () => {
    expect(isValidDate(2020, 2, 29)).toBe(true);
  });

  it("should return false for non-leap year February 29", () => {
    expect(isValidDate(2022, 2, 29)).toBe(false);
  });

  it("should return true for valid dates in different months", () => {
    expect(isValidDate(2022, 1, 1)).toBe(true);
    expect(isValidDate(2022, 2, 28)).toBe(true);
    expect(isValidDate(2022, 12, 31)).toBe(true);
    expect(isValidDate(2022, 6, 15)).toBe(true);
  });
});
